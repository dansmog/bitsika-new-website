...
import ScrollComponent from "../../../../components/legacy/ScrollComponent";
import { Loader } from "../../../../components/legacy/Loader";
import { useRouter } from "../../../../components/legacy/router-shim";
import { useLocale } from "../../../../context/locale-context";
import { getUniqueTrades } from "../../../../utils";
import translate from "../../../../translations/profile";

export default function UserTradesView({ user, err, language }) {
  ...
  const getUserTrades = async (page) => {
    const BASE_URL = process.env.NEXT_PUBLIC_TRADE_BASE_URL;
    const nextpage = page ? page : currentPage;

    setLoading(true);
    const response = await axios.get(
      `${BASE_URL}/public/trades/me?username=${user?.username}&page=${nextpage}&limit=20`,
      {
        headers: {
    ...
    newTrades.length > 0 && setTrades(getUniqueTrades(trades, newTrades));
  };
  ...
  const handleTabChange = (page) => {
    switch (selectedTab) {
      case `/user/${user?.username}/trade`:
        return !loading && getUserTrades(page);
      default:
        break;
    }
  };
                ...
                <div>
                  ...
                  >
                    {trades?.length > 0
                      ? trades.map((trade) => (
                          <P2PItem
                            onClickItem={(username, id) =>
                              push(`/user/${username}/trade/${id}`)
                            }
                            trade={trade}
                            key={trade.id}
                          />
