...
const cellBase = "px-5 py-4 text-sm text-[#505050] align-top whitespace-nowrap";
const headerBase = "px-5 py-4 text-sm font-semibold text-left whitespace-nowrap ";
      ...
      <table className="w-full border-collapse min-w-175">
        <thead>
          <tr className="bg-surface-subtle">
            ...
            <th className={`${headerBase} text-white bg-brand-blue-light border-l border-border-default`}>
              Bitsika Virtual Visa Card
            </th>
            <th className={`${headerBase} text-ink border-l border-border-default`}>
              Other Virtual Card Apps
            </th>
            <th className={`${headerBase} text-ink border-l border-border-default`}>Bank Apps</th>
            <th className={`${headerBase} text-ink border-l border-border-default`}>Bank Apps</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-t border-border-default">
              ...
              <td className={`${cellBase} bg-[#EEF6FF] border-l border-border-default`}>{row.bitsika}</td>
              <td className={`${cellBase} border-l border-border-default`}>{row.others}</td>
              <td className={`${cellBase} border-l border-border-default`}>{row.bank}</td>
              <td className={`${cellBase} border-l border-border-default`}>{row.bank}</td>
            </tr>
          ))}
        </tbody>
